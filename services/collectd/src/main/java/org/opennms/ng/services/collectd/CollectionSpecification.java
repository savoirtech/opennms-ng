...
import org.opennms.ng.services.polloutagesconfig.PollOutagesConfigFactory;
...
public class CollectionSpecification {
    ...
    public void initialize(CollectionAgent agent) throws CollectionInitializationException {
        org.opennms.netmgt.collectd.Collectd.instrumentation().beginCollectorInitialize(agent.getNodeId(), agent.getHostAddress(), m_svcName);
        try {
            m_collector.initialize(agent, getPropertyMap());
        } finally {
            org.opennms.netmgt.collectd.Collectd.instrumentation().endCollectorInitialize(agent.getNodeId(), agent.getHostAddress(), m_svcName);
        }
    }
    ...
    public void release(CollectionAgent agent) {
        org.opennms.netmgt.collectd.Collectd.instrumentation().beginCollectorRelease(agent.getNodeId(), agent.getHostAddress(), m_svcName);
        try {
            m_collector.release(agent);
        } finally {
            org.opennms.netmgt.collectd.Collectd.instrumentation().endCollectorRelease(agent.getNodeId(), agent.getHostAddress(), m_svcName);
        }
    }
    ...
    public CollectionSet collect(CollectionAgent agent) throws CollectionException {
        org.opennms.netmgt.collectd.Collectd.instrumentation().beginCollectorCollect(agent.getNodeId(), agent.getHostAddress(), m_svcName);
        try {
     ...
     *
     * @param collectionName a {@link String} object.
