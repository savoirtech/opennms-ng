...
public class PersistOperationBuilder {
     ...
     * @param rrdName a {@link String} object.
     */
    ...
    public void commit() throws RrdException {
        ...
        RrdUtils.createRRD(m_resource.getOwnerName(), getResourceDir(m_resource).getAbsolutePath(), m_rrdName, getRepository().getStep(),
            getDataSources(), getRepository().getRraList(), getAttributeMappings());
        RrdUtils.updateRRD(m_resource.getOwnerName(), getResourceDir(m_resource).getAbsolutePath(), m_rrdName, m_timeKeeper.getCurrentTime(),
            getValues());
    ...
    private List<RrdDataSource> getDataSources() {
        ...
        for (AttributeDefinition attrDef : m_declarations.keySet()) {
            ...
            RrdDataSource rrdDataSource = new RrdDataSource(StringUtils.truncate(attrDef.getName(),
                PersistOperationBuilder.MAX_DS_NAME_LENGTH), PersistOperationBuilder.mapType(
                attrDef.getType()), getRepository().getHeartBeat(), minval, maxval);
