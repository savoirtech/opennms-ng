...
public class HttpMonitor extends AbstractServiceMonitor {
    ...
    public static final Logger LOG = LoggerFactory.getLogger(org.opennms.ng.services.poller.monitors.HttpMonitor.class);
    ...
    final class HttpMonitorClient {
        ...
        public void closeConnection() {
            try {
                ...
                e.fillInStackTrace();
                org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.warn("Error closing socket connection", e);
            }
        }
            ...
            if (m_currentLine != null) {
                ...
                if (m_currentLine.startsWith("HTTP/")) {
                    ...
                    if (IPLike.matchNumericListOrRange(String.valueOf(serverResponseValue), determineResponse(m_parameters))) {
                        if (org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                            org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("determineServerResponse: valid server response: "+serverResponseValue+" found.");
                        }
        ...
        private int parseHttpResponse() {
            ...
            if (t.hasMoreTokens()) {
                try {
                    ...
                    if (org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isInfoEnabled()) {
                        org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.info("Error converting response code from host = " + (m_iface.getAddress()) + ", response = " + m_currentLine);
                    }
                }
            }
        ...
        public String readLine() throws IOException {
            ...
            if (determineVerbosity(m_parameters) && org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("\t<<: {}", m_currentLine);
            }
        ...
        public String readLinedMatching() throws IOException {
            ...
            if (m_responseText != null && m_currentLine != null && !m_responseTextFound) {
                if (checkCurrentLineMatchesResponseText()) {
                    if (org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                        org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("response-text: "+m_responseText+": found.");
                    }
                    m_serviceStatus = PollStatus.SERVICE_AVAILABLE;
                }
            }
        ...
        public void sendHttpCommand() throws IOException {
            if (determineVerbosity(m_parameters) && org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("Sending HTTP command: {}", m_httpCmd);
            }
            ...
            if (determineVerbosity(m_parameters)) {
                org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("Server response: {}", m_currentLine);
            }
        ...
        private void buildCommand() {
            ...
            if (org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("checkStatus: cmd:\n", cmd);
            }
            m_httpCmd = cmd;
        }
        ...
        protected PollStatus determinePollStatusResponse() {
            ...
            */
            ...
            if (getPollStatus() == PollStatus.SERVICE_UNAVAILABLE) {
                ...
                if (org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.isDebugEnabled()) {
                    org.opennms.ng.services.poller.monitors.HttpMonitor.LOG.debug("checkStatus: Reason: \""+getReason()+"\"");
                }
