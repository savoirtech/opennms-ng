...
public class HttpPlugin extends AbstractTcpPlugin {
    private static final Logger LOG = LoggerFactory.getLogger(org.opennms.ng.services.capsd.plugins.HttpPlugin.class);
     ...
     * <p>Constructor for HttpPlugin.</p>
     *
     * @param protocolName    a {@link String} object.
     * @param checkReturnCode a boolean.
     * @param queryString     a {@link String} object.
     * @param responseString  a {@link String} object.
     */
        ...
        try {
            ...
            if (response.toString() != null && response.toString().indexOf(m_responseString) > -1) {
                if (m_checkReturnCode) {
                    ...
                    if ((DEFAULT_URL.equals(config.getKeyedString(PROPERTY_NAME_URL, DEFAULT_URL))) || (config.getKeyedBoolean(
                        PROPERTY_NAME_RETURN_CODE, true) == false)) {
                        maxRetCode = 600;
                    }
     ...
     * (non-Javadoc)
     * 
     * @see org.opennms.ng.services.capsd.AbstractTcpPlugin#getConnectionConfigList(java.util.Map,
