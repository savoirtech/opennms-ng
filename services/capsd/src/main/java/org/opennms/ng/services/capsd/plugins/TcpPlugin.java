...
public final class TcpPlugin extends AbstractPlugin {
    private static final Logger LOG = LoggerFactory.getLogger(org.opennms.ng.services.capsd.plugins.TcpPlugin.class);
    ...
    private boolean isServer(InetAddress host, int port, int retries, int timeout, RE regex, StringBuffer bannerResult) {
        ...
        for (int attempts = 0;attempts <= retries && !isAServer;attempts++) {
            ...
            try {
                ...
                isAServer = false;
                LOG.warn("TcpPlugin: An undeclared throwable exception was caught connecting to host {} on port " + port, t, InetAddressUtils.str(
                    host));
            } finally {
                try {
                    if (socket != null) {
