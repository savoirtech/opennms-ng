...
CREATE TABLE notifications (notifyId INTEGER NOT NULL, answeredBy VARCHAR(256), eventUEI VARCHAR(255), interfaceId VARCHAR(100), notifConfigName VARCHAR(255), numericMsg VARCHAR(256), pageTime TIMESTAMP, queueId VARCHAR(256), respondTime TIMESTAMP, subject VARCHAR(256), textMsg VARCHAR(4000) NOT NULL, eventId INTEGER NOT NULL, nodeId INTEGER, serviceId INTEGER, PRIMARY KEY (notifyId));
